...
#ifndef RHEA_AST_EXPR_WHILE_HPP
#define RHEA_AST_EXPR_WHILE_HPP
...
class WhileExpression final : public ASTNode {
   ...
   public:
    explicit WhileExpression(std::shared_ptr<Token> _address,
                             std::shared_ptr<ASTNode> _expression,
                             std::shared_ptr<ASTNode> _body)
        : expression(std::move(_expression)), body(std::move(_body)) {
