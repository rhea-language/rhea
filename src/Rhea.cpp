...
auto printBanner(RheaUtil::ArgumentParser argParse) -> void {
    ...
    std::cout
        << std::endl
        << "\u001b[3;37mRhea is free software, please check the source for "
           "copying"
        << std::endl
    ...
    std::cout << std::endl
              << "\u001b[32mUsage\u001b[0m:" << std::endl
              << "  " << argParse.getProgramFileName() << " [args] <file-list>"
              << std::endl;
    ...
    std::cout << std::endl
              ...
              << std::endl
              << "License along with Rhea. If not, see:" << std::endl
              << "<https://www.gnu.org/licenses/>.\u001b[0m" << std::endl
              << std::endl;

#ifdef _OPENMP
    std::cout << "This binary is using \u001b[1mOpenMP\u001b[0m." << std::endl;
#endif
}
...
auto main(int argc, char** argv) -> int {
    ...
    argParse.defineParameter("r", "repl",
                             "Interative interpreter mode (REPL).");
    argParse.defineParameter("t", "test", "Run the script files in test mode.");
    argParse.defineParameter("u", "unsafe",
                             "Run the script files in unsafe mode.");
    ...
    if(argParse.hasParameter("t")) Runtime::setTestMode(true);

    if(argParse.hasParameter("u")) Runtime::setUnsafeMode(true);
    ...
    if(argParse.hasParameter("r")) {
        ...
        SymbolTable symbols;
        return Runtime::interpreter(symbols, argParse.getInputFiles());
    }
