...
DynamicObject UseStatement::visit(SymbolTable& symbols) {
#ifndef __EMSCRIPTEN__
    DynamicObject libName = this->libraryName->visit(symbols),
                  libVer = this->libraryVersion->visit(symbols);

    std::string lName = libName.toString(), lVersion = libVer.toString();

    if(!RheaUtil::SemVer::validateSemVer(lVersion))
        throw ASTNodeException(
            std::move(address),
            "Invalid semantic version '" + lVersion + "' for library " + lName);

    try {
        if(!RheaUtil::PathHelper::isLibraryInstalled(lName, lVersion))
            throw ASTNodeException(
                std::move(this->address),
                "Library not installed: " + lName + "@" + lVersion);
    ...
    Runtime::interpreter(
        symbols, RheaUtil::PathHelper::getLibraryFiles(lName, lVersion));
#else
    throw ASTNodeException(
        std::move(address),
