...
bool DynamicObject::operator==(const DynamicObject& other) {
    if(this->isNil() && other.isNil())
        ...
        if(len != other.getArray()->size()) return false;

        auto left = this->getArray(), right = other.getArray();
...
std::shared_ptr<FunctionDeclarationExpression> DynamicObject::getCallable()
    const {
...
bool DynamicObject::booleanEquivalent() {
    return (this->isBool() && this->getBool()) ||
           ...
           this->isFunction() || this->isRegex() || this->isNative();
}

void DynamicObject::setArrayElement(std::shared_ptr<Token> reference,
                                    size_t index,
    ...
    if(index >= this->arrayValue->size())
        throw ASTNodeException(std::move(reference), "Index is out of bounds.");

    (*this->arrayValue)[index] = std::move(*object);
}

DynamicObject DynamicObject::callFromNative(std::shared_ptr<Token> address,
                                            SymbolTable& symtab,
                                            std::vector<DynamicObject> args) {
    if(!this->isNative() && !this->isFunction())
        throw ASTNodeException(std::move(address),
                               "Cannot call non-callable object");

    return !this->isNative()
               ? this->getCallable()->call(symtab, args)
               : this->getNativeFunction()(std::move(address), symtab, args,
...
DynamicObject operator+(DynamicObject left, DynamicObject right) {
    if(left.isNumber() && right.isNumber())
        ...
        return DynamicObject(std::make_shared<RegexWrapper>(
            std::string(std::to_string(left.getNumber())) + right.toString()));
        ...
        if(RheaUtil::isNumberArray(*left.getArray()) &&
           RheaUtil::isNumberArray(*right.getArray())) {
            return DynamicObject(RheaUtil::vector2Object(
                RheaUtil::VectorMath::add(RheaUtil::object2Vector(left),
                                          RheaUtil::object2Vector(right))));
        }
        ...
        throw std::runtime_error(
            ...
            "not all elements are of number type.");
              ...
              (right.isBool() || right.isFunction() || right.isNil() ||
               right.isNumber() || right.isRegex() || right.isString())) {
        ...
        return DynamicObject(std::make_shared<RegexWrapper>(
            left.getRegex()->getPattern() + right.getRegex()->getPattern()));
    else if(left.isBool() && right.isBool())
        return DynamicObject(
            static_cast<double>(static_cast<double>(left.getBool()) +
        ...
        return DynamicObject(static_cast<double>(
            static_cast<double>(left.getBool()) + right.getNumber()));
    ...
    throw std::runtime_error("Invalid '+' operator for object types; " +
...
DynamicObject operator-(DynamicObject left, DynamicObject right) {
    if(left.isNil())
        ...
        return DynamicObject(left.getNumber() -
                             static_cast<double>(right.getBool()));
    else if(left.isBool() && right.isNumber())
        return DynamicObject(static_cast<double>(left.getBool()) -
                             right.getNumber());
    else if(((left.isString() || left.isRegex()) && right.isNumber()) ||
            (left.isNumber() && (right.isString() || right.isRegex())) ||
            (left.isString() && right.isString())) {
        std::string leftVal = left.toString(), rightVal = right.toString();
        ...
        if(RheaUtil::isNumberArray(*left.getArray()) &&
           RheaUtil::isNumberArray(*right.getArray())) {
            return DynamicObject(RheaUtil::vector2Object(
                RheaUtil::VectorMath::sub(RheaUtil::object2Vector(left),
                                          RheaUtil::object2Vector(right))));
        }
...
DynamicObject operator/(DynamicObject left, DynamicObject right) {
    if(left.isNumber() && right.isNumber()) {
        if(std::abs(right.getNumber()) < 1e-15)
            throw std::runtime_error("Division by zero.");
        return DynamicObject(left.getNumber() / right.getNumber());
    } else if(left.isNumber() && right.isBool()) {
        if(!right.getBool())
            throw std::runtime_error("Division by zero (boolean false).");
        return DynamicObject(left.getNumber() /
                             static_cast<double>(right.getBool()));
    } else if(left.isBool() && right.isNumber()) {
        if(std::abs(right.getNumber()) < 1e-15)
        ...
        if(RheaUtil::isNumberArray(*left.getArray()) &&
           RheaUtil::isNumberArray(*right.getArray())) {
            return DynamicObject(RheaUtil::vector2Object(
                RheaUtil::VectorMath::div(RheaUtil::object2Vector(left),
                                          RheaUtil::object2Vector(right))));
        }
...
DynamicObject operator*(DynamicObject left, DynamicObject right) {
    if(left.isNumber() && right.isNumber())
        ...
        const std::string& unit = left.getString();
        long count = static_cast<long>(right.getNumber());

        if(count <= 0) return DynamicObject(std::string(""));

        std::string str;
        str.reserve(unit.size() * static_cast<size_t>(count));

        for(long i = 0; i < count; i++) str += unit;
        return DynamicObject(str);
    } else if(left.isNumber() && right.isString()) {
        const std::string& unit = right.getString();
        ...
        if(RheaUtil::isNumberArray(*left.getArray()) &&
           RheaUtil::isNumberArray(*right.getArray())) {
            return DynamicObject(RheaUtil::vector2Object(
                RheaUtil::VectorMath::mul(RheaUtil::object2Vector(left),
                                          RheaUtil::object2Vector(right))));
        }
...
DynamicObject operator%(DynamicObject left, DynamicObject right) {
    if(left.isNumber() && right.isNumber()) {
        long rhs = static_cast<long>(right.getNumber());
        if(rhs == 0) throw std::runtime_error("Modulo by zero.");
        return DynamicObject(
            static_cast<double>(static_cast<long>(left.getNumber()) % rhs));
    } else if(left.isNumber() && right.isBool()) {
        long rhs = static_cast<long>(right.getBool());
        if(rhs == 0)
            throw std::runtime_error("Modulo by zero (boolean false).");
        return DynamicObject(
            static_cast<double>(static_cast<long>(left.getNumber()) % rhs));
    } else if(left.isBool() && right.isNumber()) {
        long rhs = static_cast<long>(right.getNumber());
        if(rhs == 0) throw std::runtime_error("Modulo by zero.");
        return DynamicObject(
            static_cast<double>(static_cast<long>(left.getBool()) % rhs));
    } else if(left.isBool() && right.isBool()) {
        long rhs = static_cast<long>(right.getBool());
        if(rhs == 0)
            throw std::runtime_error("Modulo by zero (boolean false).");
        ...
        if(RheaUtil::isNumberArray(*left.getArray()) &&
           RheaUtil::isNumberArray(*right.getArray())) {
            return DynamicObject(RheaUtil::vector2Object(
                RheaUtil::VectorMath::rem(RheaUtil::object2Vector(left),
                                          RheaUtil::object2Vector(right))));
        }
...
DynamicObject operator<(DynamicObject left, DynamicObject right) {
    if(left.isNumber() && right.isNumber())
        ...
        return DynamicObject(left.getString().size() <
                             static_cast<std::size_t>(right.getNumber()));
    else if(left.isNumber() && right.isString())
        return DynamicObject(static_cast<std::size_t>(left.getNumber()) <
...
DynamicObject operator<<(DynamicObject left, DynamicObject right) {
    auto guardShift = [](long shift) {
        if(shift < 0 || shift >= static_cast<long>(sizeof(long) * 8))
    ...
    if(left.isNumber() && right.isNumber()) {
        ...
        if(RheaUtil::isNumberArray(*left.getArray()) &&
           RheaUtil::isNumberArray(*right.getArray())) {
            return DynamicObject(
                RheaUtil::vector2Object(RheaUtil::VectorMath::shiftLeft(
                    RheaUtil::object2Vector(left),
                    RheaUtil::object2Vector(right))));
        }
...
DynamicObject operator>>(DynamicObject left, DynamicObject right) {
    ...
    if(left.isNumber() && right.isNumber()) {
        ...
        if(RheaUtil::isNumberArray(*left.getArray()) &&
           RheaUtil::isNumberArray(*right.getArray())) {
            return DynamicObject(
                RheaUtil::vector2Object(RheaUtil::VectorMath::shiftRight(
                    RheaUtil::object2Vector(left),
                    RheaUtil::object2Vector(right))));
        }
...
DynamicObject operator&(DynamicObject left, DynamicObject right) {
    if(left.isNumber() && right.isNumber())
        return DynamicObject(
            static_cast<double>(static_cast<long>(left.getNumber()) &
        ...
        return DynamicObject(
            static_cast<double>(static_cast<long>(left.getBool()) &
                                static_cast<long>(right.getNumber())));
    else if(left.isBool() && right.isBool())
        return DynamicObject(
            static_cast<double>(static_cast<long>(left.getBool()) &
        ...
        if(RheaUtil::isNumberArray(*left.getArray()) &&
           RheaUtil::isNumberArray(*right.getArray())) {
            return DynamicObject(
                RheaUtil::vector2Object(RheaUtil::VectorMath::bitwiseAnd(
                    RheaUtil::object2Vector(left),
                    RheaUtil::object2Vector(right))));
        }
...
DynamicObject operator|(DynamicObject left, DynamicObject right) {
    if(left.isNumber() && right.isNumber())
        ...
        if(RheaUtil::isNumberArray(*left.getArray()) &&
           RheaUtil::isNumberArray(*right.getArray())) {
            return DynamicObject(
                RheaUtil::vector2Object(RheaUtil::VectorMath::bitwiseOr(
                    RheaUtil::object2Vector(left),
                    RheaUtil::object2Vector(right))));
        }
...
DynamicObject operator^(DynamicObject left, DynamicObject right) {
    if(left.isNumber() && right.isNumber())
        ...
        if(RheaUtil::isNumberArray(*left.getArray()) &&
           RheaUtil::isNumberArray(*right.getArray())) {
            return DynamicObject(
                RheaUtil::vector2Object(RheaUtil::VectorMath::bitwiseXor(
                    RheaUtil::object2Vector(left),
                    RheaUtil::object2Vector(right))));
        }
...
DynamicObject operator&&(DynamicObject left, DynamicObject right) {
    return DynamicObject(left.booleanEquivalent() && right.booleanEquivalent());
}
...
DynamicObject DynamicObject::vectorAdd(DynamicObject arrayVal) {
    ...
    if(!RheaUtil::isNumberArray(*arrayVal.getArray()))
        throw std::runtime_error(
            "Object elements are not of number array type.");

    return DynamicObject(
        RheaUtil::vector2Object(RheaUtil::VectorMath::addSingle(
            this->getNumber(), RheaUtil::object2Vector(arrayVal))));
}
...
DynamicObject DynamicObject::vectorSub(DynamicObject arrayVal) {
    ...
    return DynamicObject(
        RheaUtil::vector2Object(RheaUtil::VectorMath::subSingle(
            this->getNumber(), RheaUtil::object2Vector(arrayVal))));
}
...
DynamicObject DynamicObject::vectorDiv(DynamicObject arrayVal) {
    ...
    return DynamicObject(
        RheaUtil::vector2Object(RheaUtil::VectorMath::divSingle(
            this->getNumber(), RheaUtil::object2Vector(arrayVal))));
}
...
DynamicObject DynamicObject::vectorMul(DynamicObject arrayVal) {
    ...
    return DynamicObject(
        RheaUtil::vector2Object(RheaUtil::VectorMath::mulSingle(
            this->getNumber(), RheaUtil::object2Vector(arrayVal))));
}
...
DynamicObject DynamicObject::vectorRem(DynamicObject arrayVal) {
    ...
    return DynamicObject(
        RheaUtil::vector2Object(RheaUtil::VectorMath::remSingle(
            this->getNumber(), RheaUtil::object2Vector(arrayVal))));
}
...
DynamicObject DynamicObject::vectorBitwiseAnd(DynamicObject arrayVal) {
    ...
    return DynamicObject(
        RheaUtil::vector2Object(RheaUtil::VectorMath::bitwiseAndSingle(
            this->getNumber(), RheaUtil::object2Vector(arrayVal))));
}
...
DynamicObject DynamicObject::vectorBitwiseOr(DynamicObject arrayVal) {
    ...
    return DynamicObject(
        RheaUtil::vector2Object(RheaUtil::VectorMath::bitwiseOrSingle(
            this->getNumber(), RheaUtil::object2Vector(arrayVal))));
}
...
DynamicObject DynamicObject::vectorBitwiseXor(DynamicObject arrayVal) {
    ...
    return DynamicObject(
        RheaUtil::vector2Object(RheaUtil::VectorMath::bitwiseXorSingle(
            this->getNumber(), RheaUtil::object2Vector(arrayVal))));
}
...
DynamicObject DynamicObject::vectorShiftLeft(DynamicObject arrayVal) {
    ...
    return DynamicObject(
        RheaUtil::vector2Object(RheaUtil::VectorMath::shiftLeftSingle(
            this->getNumber(), RheaUtil::object2Vector(arrayVal))));
}
...
DynamicObject DynamicObject::vectorShiftRight(DynamicObject arrayVal) {
    ...
    return DynamicObject(
        RheaUtil::vector2Object(RheaUtil::VectorMath::shiftRightSingle(
            this->getNumber(), RheaUtil::object2Vector(arrayVal))));
}
