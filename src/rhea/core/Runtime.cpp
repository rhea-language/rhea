...
void Runtime::addLoadedLibrary(std::string libName, void* handle) {
    ...
    std::lock_guard<std::mutex> lock(Runtime::runtimeMtx);
...
void* Runtime::getLoadedLibrary(std::string libName) {
    ...
    return it == Runtime::nativeLibraries.end() ? nullptr : it->second;
#else
    (void)libName;
    return nullptr;
#endif
...
bool Runtime::hasFileHash(std::string hash) {
    std::lock_guard<std::mutex> lock(Runtime::runtimeMtx);
    auto begin = Runtime::fileHashes.begin(), end = Runtime::fileHashes.end();
...
#ifndef __EMSCRIPTEN__
    std::lock_guard<std::mutex> lock(Runtime::runtimeMtx);
    if(Runtime::nativeLibraries.empty()) return;
...
int Runtime::interpreter(SymbolTable& symbols, std::vector<std::string> files) {
    try {
        ...
        parsync(iterator = files.begin(); iterator != files.end(); iterator++) {
            ...
            if(Runtime::hasFileHash(fileHash)) continue;
...
void Runtime::showPrompt() {
    ...
    std::cout << std::endl;
    std::cout << "\u001b[1;34m\u250c─\u001b[0m [ \u001b[3;33m" << workingDir
              << "\u001b[0m ]" << std::endl;
    std::cout << "\u001b[1;34m\u251c─\u001b[0m [ \u001b[33m"
              << std::put_time(&localTime, "%m/%d/%Y %H:%M:%S") << "\u001b[0m ]"
              << std::endl;
}
...
void Runtime::repl() {
    ...
    while(true) {
        ...
        try {
            Tokenizer tokenizer(
                input, "<repl, iteration: " + std::to_string(iterNum) + ">");
            tokenizer.scan();
...
void Runtime::segfaultHandler(int signal, siginfo_t* si,
    ...
    std::cout << "\u001b[1;41m     Segmentation Fault     \u001b[0m"
              << std::endl;
    std::cout << "\u001b[1;31mCaught signal\u001b[0m:\t\t0x" << std::hex
              << std::setw(2) << std::setfill('0') << signal << std::dec
              << "\n";
    std::cout << "\u001b[1;31mFaulting address\u001b[0m:\t0x" << std::hex
              << std::setw(2) << std::setfill('0')
              << reinterpret_cast<std::uintptr_t>(si->si_addr) << std::dec
              << "\n";
...
void Runtime::terminateHandler() {
    ...
    if(!eptr) {
        RheaUtil::renderError(
            ...
            "\033[3;37mTerminated with no active exception.\033[0m\r\n");
        std::abort();
    }
