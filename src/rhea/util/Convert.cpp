...
double Convert::toDouble(const unsigned char* bytes, size_t length) {
    if(bytes == nullptr || length != 8)
        throw std::invalid_argument(
            "Byte array must be non-null and have length 8");
...
double Convert::translateDigit(const std::string& image) {
    ...
    if(image.find("0b") == 0) {
        ...
        if(digits.empty() || !isValidForBase(digits, 2))
            throw std::invalid_argument("Invalid binary literal: '" + image +
                                        "'");
        ...
        if(digits.empty() || !isValidForBase(digits, 3))
            throw std::invalid_argument("Invalid trinary literal: '" + image +
                                        "'");
        ...
        if(digits.empty() || !isValidForBase(digits, 8))
            throw std::invalid_argument("Invalid octadecimal literal: '" +
                                        image + "'");
    ...
    try {
        ...
        throw std::invalid_argument("Numeric literal out of range: '" + image +
                                    "'");
    }
}
...
double Convert::parseBinary(const std::string& str) {
    try {
        return static_cast<double>(std::stoll(str, nullptr, 2));
    } catch(const std::out_of_range&) {
        throw std::invalid_argument("Binary literal out of range: '" + str +
                                    "'");
    }
}
...
double Convert::parseBase3(const std::string& str) {
    try {
        return static_cast<double>(std::stoll(str, nullptr, 3));
    } catch(const std::out_of_range&) {
        throw std::invalid_argument("Trinary literal out of range: '" + str +
                                    "'");
    }
}
...
double Convert::parseOctal(const std::string& str) {
    try {
        return static_cast<double>(std::stoll(str, nullptr, 8));
    } catch(const std::out_of_range&) {
        throw std::invalid_argument("Octal literal out of range: '" + str +
                                    "'");
    }
}
...
double Convert::parseHex(const std::string& str) {
    try {
        ...
        throw std::invalid_argument("Hexadecimal literal out of range: '" +
                                    str + "'");
    }
}
