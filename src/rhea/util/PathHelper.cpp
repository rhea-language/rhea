...
std::string PathHelper::wstring2Utf8(const std::wstring& wstr) {
    ...
    std::string utf8Str(static_cast<size_t>(sizeNeeded), 0);
    int charsConverted =
    ...
    if(charsConverted == 0)
        throw std::runtime_error(
            "Path helper failed to convert wstring to UTF-8.");

    utf8Str.resize(static_cast<size_t>(charsConverted));
...
std::string PathHelper::installationPath() {
    ...
    if(RegOpenKeyExW(HKEY_CURRENT_USER, L"Environment", 0, KEY_READ, &hKey) ==
       ERROR_SUCCESS) {
        if(RegQueryValueExW(hKey, L"RHEA_PATH", NULL, &type,
                            reinterpret_cast<LPBYTE>(buffer),
                            &buffer_size_bytes) == ERROR_SUCCESS) {
            if(type == REG_SZ || type == REG_EXPAND_SZ) {
                ...
                if(num_wchars > 0 && buffer[num_wchars - 1] == L'\0')
                    ...
                    if(buffer[32767 - 1] != L'\0') buffer[32767 - 1] = L'\0';
                }
    ...
    if(env_path != nullptr) return std::string(env_path);

    return "";

#else
    const char* envPath = std::getenv(RHEA_ENV_PATH_NAME);
    return envPath ? std::string(envPath) : std::string("");
#endif
}

bool PathHelper::isLibraryInstalled(std::string libraryName,
                                    std::string libraryVersion) {
    namespace fs = std::filesystem;
    std::string path = PathHelper::installationPath() + FS_FILE_SEPARATOR +
                       "modules" + FS_FILE_SEPARATOR + libraryName + "@" +
                       libraryVersion + FS_FILE_SEPARATOR + "src";
...
std::string PathHelper::findSharedLibrary(std::string name) {
    ...
    if(std::filesystem::exists(rawName)) return rawName;

    namespace fs = std::filesystem;
    std::string libPath =
        PathHelper::installationPath() + FS_FILE_SEPARATOR + "modules";

    for(const auto& modules : std::filesystem::directory_iterator(libPath)) {
        std::string modPath =
            modules.path().string() + FS_FILE_SEPARATOR + "lib";
        std::string folderName = fs::path(modules).filename().string();

        if(folderName.rfind(name, 0) == 0)
            for(const auto& entry :
                std::filesystem::directory_iterator(modPath))
