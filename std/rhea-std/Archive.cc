...
RHEA_FUNC(archive_zip_open) {
    ...
    DynamicObject path = args.at(0), flags = args.at(1);
    std::vector<DynamicObject> values = {};

    if(!flags.isNumber())
        throw TerminativeThrowSignal(
            std::move(address),
            "Expecting number argument for flags, got " + flags.objectType());

    int error = 0;
    zip_t* zip = zip_open(path.toString().c_str(),
                          static_cast<int>(flags.getNumber()), &error);

    std::string key = RheaUtil::uniqueKey();
    if(zip == NULL || error != 0) {
        values.emplace_back(DynamicObject(key));
        values.emplace_back(DynamicObject(static_cast<double>(error)));
    ...
    {
        std::lock_guard<std::mutex> lock(zipMtx);
        zipMap[key] = zip;
    }
...
RHEA_FUNC(archive_zip_close) {
    ...
    zip_t* zip = it->second;
    if(zip == NULL)
        throw TerminativeThrowSignal(std::move(address), "Invalid zip handle");

    int result = zip_close(zip);
    zipMap.erase(it);
    return DynamicObject(static_cast<double>(result));
}
...
RHEA_FUNC(archive_zip_compressionMethod) {
    ...
    DynamicObject method = args.at(0), compress = args.at(1);
    ...
    return DynamicObject(zip_compression_method_supported(
                             ...
                             static_cast<int>(compress.getNumber())) == 1);
}
...
RHEA_FUNC(archive_zip_setPassword) {
    ...
    DynamicObject key = args.at(0), password = args.at(1);
    std::string keyStr = key.toString();
    std::lock_guard<std::mutex> lock(zipMtx);
    ...
    return DynamicObject(zip_set_default_password(
                             zipMap[keyStr], password.toString().c_str()) == 0);
}
...
RHEA_FUNC(archive_zip_setEncryption) {
    ...
    DynamicObject key = args.at(0), index = args.at(1), method = args.at(2),
                  password = args.at(3);
    std::string keyStr = key.toString();
...
RHEA_FUNC(archive_zip_addFromData) {
    ...
    if(args.size() != 5)
        throw TerminativeThrowSignal(
            std::move(address),
            "Expecting 5 argument, got " + std::to_string(args.size()));

    DynamicObject key = args.at(0), flags = args.at(1),
                  compression = args.at(2), fileName = args.at(3),
                  buffer = args.at(4);
    std::string keyStr = key.toString();
    ...
    zip_source_t* src = zip_source_buffer(zip, data, dataSize, 1);
    ...
    if(src == NULL) {
        ...
        values.emplace_back(DynamicObject(std::string(zip_strerror(zip))));

        return DynamicObject(
            std::make_shared<std::vector<DynamicObject>>(values));
    }

    zip_int64_t idx = zip_file_add(zip, fileName.toString().c_str(), src,
                                   static_cast<uint32_t>(flags.getNumber()));

    if(idx < 0) {
        zip_source_free(src);

        values.emplace_back(DynamicObject(static_cast<double>(idx)));
        values.emplace_back(DynamicObject(std::string(zip_strerror(zip))));
    } else {
        int comp = zip_set_file_compression(
            zip, idx, static_cast<int32_t>(compression.getNumber()), 0);

        values.emplace_back(DynamicObject(static_cast<float>(idx)));
...
RHEA_FUNC(archive_zip_addFromFile) {
    ...
    zip_source_t* src =
        zip_source_file(zip, originFile.toString().c_str(), 0, 0);
...
RHEA_FUNC(archive_zip_addFromString) {
    ...
    zip_source_t* src = zip_source_buffer(zip, dataCopy, dataSize, 1);
...
RHEA_FUNC(archive_zip_deleteFile) {
    ...
    DynamicObject key = args.at(0), fileName = args.at(1);
    std::string keyStr = key.toString();
    ...
    int64_t index = zip_name_locate(zip, fileName.toString().c_str(), 0);

    return index < 0 ? DynamicObject(false)
...
RHEA_FUNC(archive_zip_deleteDir) {
    ...
    DynamicObject key = args.at(0), dirName = args.at(1);
    std::string keyStr = key.toString();
    ...
    for(int64_t i = num_entries - 1; i >= 0; i--) {
        ...
        if(std::string(name).rfind(dirName.toString()) != 0) continue;

        if(zip_delete(zip, i) != 0) return DynamicObject(false);
    }
...
RHEA_FUNC(archive_zip_getIndex) {
    ...
    DynamicObject key = args.at(0), fileName = args.at(1), flags = args.at(2);
    std::string keyStr = key.toString();
    ...
    return DynamicObject(static_cast<float>(
        zip_name_locate(zip, fileName.toString().c_str(),
...
RHEA_FUNC(archive_zip_hasFile) {
    ...
    return DynamicObject(zip_name_locate(zip, fileName.toString().c_str(),
                                         static_cast<int>(flags.getNumber())) >=
                         0);
}
...
RHEA_FUNC(archive_zip_renameFile) {
    ...
    DynamicObject key = args.at(0), index = args.at(1), newName = args.at(2),
                  flags = args.at(3);
    std::string keyStr = key.toString();
    ...
    return DynamicObject(
        zip_file_rename(zip, static_cast<uint64_t>(index.getNumber()),
                        newName.toString().c_str(),
                        static_cast<int>(flags.getNumber())) == 0);
}
...
RHEA_FUNC(archive_zip_entryCount) {
    ...
    return DynamicObject(static_cast<float>(
        zip_get_num_entries(zip, static_cast<int>(flags.getNumber()))));
}
...
RHEA_FUNC(archive_zip_listEntries) {
    ...
    for(size_t i = 0; i < entryCount; i++) {
        const char* name =
            zip_get_name(zip, i, static_cast<int>(flags.getNumber()));

        if(!name) continue;

        entries->emplace_back(DynamicObject(std::string(name)));
    }
...
RHEA_FUNC(archive_zip_setComment) {
    ...
    DynamicObject key = args.at(0), comment = args.at(1);
    std::string keyStr = key.toString();
    ...
    return DynamicObject(zip_set_archive_comment(zip, commentStr.c_str(),
                                                 commentStr.size()) == 0);
}
...
RHEA_FUNC(archive_zip_getComment) {
    ...
    int lenp = 0;
    return DynamicObject(std::string(zip_get_archive_comment(
        zip, &lenp, static_cast<int>(flags.getNumber()))));
}
...
RHEA_FUNC(archive_zip_getFileComment) {
    ...
    DynamicObject key = args.at(0), index = args.at(1), flags = args.at(2);
    std::string keyStr = key.toString();
    ...
    return DynamicObject(std::string(
        zip_get_file_comment(zip, static_cast<uint64_t>(index.getNumber()),
                             &lenp, static_cast<int>(flags.getNumber()))));
}
...
RHEA_FUNC(archive_zip_setFileComment) {
    ...
    DynamicObject key = args.at(0), index = args.at(1), comment = args.at(2);
    std::string keyStr = key.toString();
    ...
    std::string commentStr = comment.toString();
    return DynamicObject(
        zip_set_file_comment(zip, static_cast<uint64_t>(index.getNumber()),
                             commentStr.c_str(), commentStr.size()) == 0);
}
...
RHEA_FUNC(archive_zip_fileDosTime) {
    ...
    DynamicObject key = args.at(0), index = args.at(1), dosTime = args.at(2),
                  dosDate = args.at(3), flags = args.at(4);
    std::string keyStr = key.toString();
    ...
    return DynamicObject(
        zip_file_set_dostime(zip, static_cast<uint64_t>(index.getNumber()),
...
RHEA_FUNC(archive_zip_registerProgressCallback) {
    ...
    DynamicObject key = args.at(0), precision = args.at(1),
                  callback = args.at(2);
    std::string keyStr = key.toString();
...
RHEA_FUNC(archive_zip_readAsData) {
    ...
    struct zip_stat st;
    if(zip_stat(zip, actualFileName, 0, &st) != 0)
        throw TerminativeThrowSignal(
            std::move(address),
            "Cannot find specified file name in zip: " + fileName.toString());

    zip_int64_t idx = zip_name_locate(zip, actualFileName, 0);
    if(idx < 0)
        throw TerminativeThrowSignal(
            std::move(address),
            "Cannot read specified file name in zip: " + fileName.toString());
    ...
    if(bytes_read < 0 || static_cast<uint64_t>(bytes_read) != st.size) {
        free(buffer);
        zip_fclose(zf);
    ...
    for(size_t i = 0; i < st.size; i++)
        buf->emplace_back(DynamicObject(static_cast<float>(buffer[i])));
...
RHEA_FUNC(archive_zip_readAsString) {
    ...
    std::string buf;
    for(size_t i = 0; i < st.size; i++) buf += static_cast<char>(buffer[i]);
...
RHEA_FUNC(archive_zip_fetchInfo) {
    ...
    infos->emplace_back(
        DynamicObject(static_cast<float>(st.encryption_method)));
...
RHEA_FUNC(archive_zip_unchange) {
    ...
    DynamicObject key = args.at(0), index = args.at(1);
    std::string keyStr = key.toString();

    if(!index.isNumber())
        throw TerminativeThrowSignal(
            std::move(address),
            "Index argument must be of number type, got " + index.objectType());
    ...
    return DynamicObject(
        zip_unchange(zip, static_cast<uint64_t>(index.getNumber())) == 0);
}
