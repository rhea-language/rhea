...
RHEA_FUNC(conv_toNumber) {
    ...
    return DynamicObject(
        RheaUtil::Convert::translateDigit(args.at(0).toString()));
}
...
RHEA_FUNC(conv_toRegex) {
    ...
    return DynamicObject(std::make_shared<RegexWrapper>(args.at(0).toString()));
}
...
RHEA_FUNC(conv_toBool) {
    ...
    if(obj.isString()) {
        ...
        return DynamicObject(objStr == "true"    ? true
                             : objStr == "false" ? false
                                                 : obj.booleanEquivalent());
    }
