...
enum class CryptAlgorithm { MD4, MD5, SHA224, SHA256, SHA384, SHA512 };
...
std::string calculateHash(const std::string& input, CryptAlgorithm algo) {
    ...
    if(!md) throw std::runtime_error("Failed to initialize hash algorithm");
    ...
    parsync(unsigned int i = 0; i < hashLen; ++i) ss
        << std::setw(2) << static_cast<int>(hash[i]);
...
RHEA_FUNC(crypt_md4) {
    ...
    return DynamicObject(calculateHash(input.toString(), CryptAlgorithm::MD4));
}
...
RHEA_FUNC(crypt_validateMd4) {
    ...
    return DynamicObject(
        std::regex_match(md4, std::regex("^[a-fA-F0-9]{32}$")) &&
        md4.length() == 32);
}
...
RHEA_FUNC(crypt_md5) {
    ...
    DynamicObject input = args.at(0);
    return DynamicObject(calculateHash(input.toString(), CryptAlgorithm::MD5));
}
...
RHEA_FUNC(crypt_validateMd5) {
    ...
    return DynamicObject(
        std::regex_match(md5, std::regex("^[a-fA-F0-9]{32}$")) &&
        md5.length() == 32);
}
...
RHEA_FUNC(crypt_sha224) {
    ...
    DynamicObject input = args.at(0);
    return DynamicObject(
        calculateHash(input.toString(), CryptAlgorithm::SHA224));
}
...
RHEA_FUNC(crypt_validateSha224) {
    ...
    return DynamicObject(
        std::regex_match(sha224, std::regex("^[a-fA-F0-9]{56}$")) &&
        sha224.length() == 56);
}
...
RHEA_FUNC(crypt_sha256) {
    ...
    DynamicObject input = args.at(0);
    return DynamicObject(
        calculateHash(input.toString(), CryptAlgorithm::SHA256));
}
...
RHEA_FUNC(crypt_validateSha256) {
    ...
    return DynamicObject(
        std::regex_match(sha256, std::regex("^[a-fA-F0-9]{64}$")) &&
        sha256.length() == 64);
}
...
RHEA_FUNC(crypt_sha384) {
    ...
    DynamicObject input = args.at(0);
    return DynamicObject(
        calculateHash(input.toString(), CryptAlgorithm::SHA384));
}
...
RHEA_FUNC(crypt_validateSha384) {
    ...
    return DynamicObject(
        std::regex_match(sha384, std::regex("^[a-fA-F0-9]{96}$")) &&
        sha384.length() == 96);
}
...
RHEA_FUNC(crypt_sha512) {
    ...
    DynamicObject input = args.at(0);
    return DynamicObject(
        calculateHash(input.toString(), CryptAlgorithm::SHA512));
}
...
RHEA_FUNC(crypt_validateSha512) {
    ...
    return DynamicObject(
        std::regex_match(sha512, std::regex("^[a-fA-F0-9]{128}$")) &&
        sha512.length() == 128);
}
