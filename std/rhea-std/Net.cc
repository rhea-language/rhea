...
DynamicObject httpResponseToObject(quoneq_http_response response) {
    ...
    object.emplace_back(DynamicObject(static_cast<double>(response.status)));
    ...
    for(const auto& header : response.header) {
        ...
        headers.emplace_back(DynamicObject(
            std::make_shared<std::vector<DynamicObject>>(headerEntry)));
    }

    object.emplace_back(
        DynamicObject(std::make_shared<std::vector<DynamicObject>>(headers)));
    ...
    for(const auto& cookie : response.cookies) {
        ...
        cookies.emplace_back(DynamicObject(
            std::make_shared<std::vector<DynamicObject>>(cookieEntry)));
    }

    object.emplace_back(
        DynamicObject(std::make_shared<std::vector<DynamicObject>>(cookies)));

    return DynamicObject(std::make_shared<std::vector<DynamicObject>>(object));
}

std::pair<bool, std::map<std::string, std::string>> objectArrayToMap(
...
std::pair<bool, std::vector<std::string>> objectArray1DToMap(
    DynamicObject object) {
    if(!object.isArray())
...
RHEA_FUNC(net_http_get) {
    ...
    if(args.size() < 1 || args.size() > 6)
        throw TerminativeThrowSignal(
            std::move(address),
            "Expecting 1 to 6 argument(s), got " + std::to_string(args.size()));

    DynamicObject urlStr = args.at(0);
    std::map<std::string, std::string> headers = {}, cookies = {};
    std::string proxy = "", username = "", password = "";

    if(args.size() >= 2) {
        auto result = objectArrayToMap(args.at(1));
        if(!result.first)
            throw TerminativeThrowSignal(std::move(address),
                                         "Header map should be of array of "
                                         "string arrays (size=2) type.");
    ...
    if(args.size() >= 3) {
        ...
        if(!result.first)
            throw TerminativeThrowSignal(std::move(address),
                                         "Cookie map should be of array of "
                                         "string arrays (size=2) type.");

        cookies = result.second;
    }

    if(args.size() >= 4) proxy = args.at(3).toString();
    if(args.size() >= 5) username = args.at(4).toString();
    if(args.size() >= 6) password = args.at(5).toString();

    return httpResponseToObject(*(quoneq_http_client::get(
        urlStr.toString(), headers, cookies, proxy, username, password)));
}
...
RHEA_FUNC(net_http_post) {
    ...
    if(args.size() < 1 || args.size() > 8)
        throw TerminativeThrowSignal(
            std::move(address),
            "Expecting 1 to 8 argument(s), got " + std::to_string(args.size()));

    DynamicObject urlStr = args.at(0);
    std::map<std::string, std::string> forms = {}, headers = {}, cookies = {},
                                       files = {};
    std::string proxy = "", username = "", password = "";
    ...
    if(args.size() >= 2) {
        ...
        if(!result.first)
            throw TerminativeThrowSignal(std::move(address),
                                         "Form data map should be of array of "
                                         "string arrays (size=2) type.");
    ...
    if(args.size() >= 6) proxy = args.at(5).toString();
    if(args.size() >= 7) username = args.at(6).toString();
    if(args.size() >= 8) password = args.at(7).toString();

    return httpResponseToObject(
        *(quoneq_http_client::post(urlStr.toString(), forms, headers, cookies,
                                   files, proxy, username, password)));
}
...
RHEA_FUNC(net_http_ping) {
    ...
    if(args.size() < 1 || args.size() > 4)
        throw TerminativeThrowSignal(
            std::move(address),
            "Expecting 1 to 4 argument(s), got " + std::to_string(args.size()));

    DynamicObject urlStr = args.at(0);
    std::string proxy = "", username = "", password = "";

    if(args.size() >= 2) proxy = args.at(1).toString();
    if(args.size() >= 3) username = args.at(2).toString();
    if(args.size() >= 4) password = args.at(3).toString();

    return httpResponseToObject(*(quoneq_http_client::ping(
        urlStr.toString(), proxy, username, password)));
}
...
RHEA_FUNC(net_http_downloadFile) {
    ...
    if(args.size() < 2 || args.size() > 9)
        throw TerminativeThrowSignal(
            std::move(address),
            "Expecting 2 to 9 argument(s), got " + std::to_string(args.size()));

    DynamicObject urlStr = args.at(0), outputFile = args.at(1);
    std::map<std::string, std::string> forms = {}, headers = {}, cookies = {},
                                       files = {};
    std::string proxy = "", username = "", password = "";
    ...
    if(args.size() >= 7) proxy = args.at(6).toString();
    if(args.size() >= 8) username = args.at(7).toString();
    if(args.size() >= 9) password = args.at(8).toString();

    return httpResponseToObject(*(quoneq_http_client::download_file(
        urlStr.toString(), outputFile.toString(), forms, headers, cookies,
        files, proxy, username, password)));
}
...
RHEA_FUNC(net_tor_get) {
    ...
    if(args.size() < 1 || args.size() > 5)
        throw TerminativeThrowSignal(
            std::move(address),
            "Expecting 1 to 5 argument(s), got " + std::to_string(args.size()));
    ...
    if(args.size() >= 4) username = args.at(3).toString();
    if(args.size() >= 5) password = args.at(4).toString();

    return httpResponseToObject(*(quoneq_tor_client::get(
        urlStr.toString(), headers, cookies, username, password)));
}
...
RHEA_FUNC(net_tor_post) {
    ...
    if(args.size() >= 6) username = args.at(5).toString();
    if(args.size() >= 7) password = args.at(6).toString();

    return httpResponseToObject(
        *(quoneq_tor_client::post(urlStr.toString(), forms, headers, cookies,
...
RHEA_FUNC(net_tor_ping) {
    ...
    if(args.size() < 1 || args.size() > 3)
        throw TerminativeThrowSignal(
            std::move(address),
            "Expecting 1 to 3 argument(s), got " + std::to_string(args.size()));

    DynamicObject urlStr = args.at(0);
    std::string username = "", password = "";

    if(args.size() >= 2) username = args.at(1).toString();
    if(args.size() >= 3) password = args.at(2).toString();

    return httpResponseToObject(
        *(quoneq_tor_client::ping(urlStr.toString(), username, password)));
}
...
RHEA_FUNC(net_tor_downloadFile) {
    ...
    if(args.size() < 2 || args.size() > 8)
        throw TerminativeThrowSignal(
            std::move(address),
            "Expecting 2 to 8 argument(s), got " + std::to_string(args.size()));
...
RHEA_FUNC(net_smtp_sendMail) {
    ...
    if(args.size() != 7)
        throw TerminativeThrowSignal(
            std::move(address),
            "Expecting 7 argument, got " + std::to_string(args.size()));

    std::string server = args.at(0).toString(), email = args.at(1).toString(),
    ...
    return DynamicObject(quoneq_smtp_client::send_mail(
        server, email, password, recipient, subject, message, files));
}
