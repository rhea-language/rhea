...
RHEA_FUNC(regex_match) {
    ...
    DynamicObject pattern = args.at(0), text = args.at(1);
    ...
    try {
        return DynamicObject(
            std::regex_match(text.toString(), pattern.getRegex()->getRegex()));
    } catch(const std::regex_error& e) {
        throw TerminativeThrowSignal(
            std::move(address), "Matching error: " + std::string(e.what()));
    }
...
RHEA_FUNC(regex_escapeRegex) {
    ...
    std::string origin = pattern.getRegex()->getPattern();
    const std::string metaChars = R"(\.^$|()[]{}*+?)";
    std::string escaped;

    escaped.reserve(origin.size() * 2);
    for(char ch : origin) {
        if(metaChars.find(ch) != std::string::npos) escaped.push_back('\\');
...
RHEA_FUNC(regex_search) {
    ...
    try {
        ...
        std::regex_search(textStr, match, pattern.getRegex()->getRegex());
...
RHEA_FUNC(regex_countMatches) {
    ...
    try {
        ...
        auto begin = std::sregex_iterator(textStr.begin(), textStr.end(), expr);
    ...
    return DynamicObject(static_cast<float>(count));
}
...
RHEA_FUNC(regex_findMatchPositions) {
    ...
    try {
        ...
        auto start = std::sregex_iterator(textStr.begin(), textStr.end(), expr);
        auto end = std::sregex_iterator();
        ...
        for(std::sregex_iterator i = start; i != end; ++i) {
            ...
            pos->emplace_back(
                DynamicObject(static_cast<float>(match.position())));
            pos->emplace_back(
                DynamicObject(static_cast<float>(match.length())));
...
RHEA_FUNC(regex_findAllMatches) {
    ...
    try {
        ...
        for(std::sregex_iterator i = start; i != end; ++i) {
            std::smatch match = *i;
            positions->emplace_back(DynamicObject(match.str()));
        }
    } catch(const std::regex_error& err) {
        throw TerminativeThrowSignal(std::move(address),
                                     std::string(err.what()));
...
RHEA_FUNC(regex_replaceAll) {
    ...
    std::regex expr = pattern.getRegex()->getRegex();
    return DynamicObject(
        std::regex_replace(text.toString(), expr, replacement.toString()));
}
...
RHEA_FUNC(regex_replaceFirst) {
    ...
    if(std::regex_search(textOutput, match, expr)) {
        size_t pos = match.position(), len = match.length();

        textOutput = textOutput.substr(0, pos) + replacement.toString() +
...
RHEA_FUNC(regex_split) {
    ...
    std::sregex_token_iterator iter(textStr.begin(), textStr.end(), expr);
    std::sregex_token_iterator end;

    for(; iter != end; ++iter)
        if(!iter->str().empty())
            positions->emplace_back(DynamicObject(iter->str()));
...
RHEA_FUNC(regex_getCapturedGroups) {
    ...
    if(std::regex_search(textStr, match, expr))
        for(size_t i = 1; i < match.size(); ++i)
            groups->push_back(DynamicObject(match[i].str()));
