...
RHEA_FUNC(sys_quickShell) {
    ...
    return DynamicObject(static_cast<double>(system(value.toString().c_str())));
}
...
RHEA_FUNC(sys_shellWrite) {
    ...
    DynamicObject value = args.at(0), content = args.at(1);
...
RHEA_FUNC(sys_shellReadOutput) {
    ...
    return DynamicObject(shellMap[uuid]->readShellOutputStream());
}
...
RHEA_FUNC(sys_shellReadError) {
    ...
    return DynamicObject(shellMap[uuid]->readShellErrorStream());
}
...
RHEA_FUNC(sys_shellExitCode) {
    ...
    return DynamicObject(static_cast<double>(shellMap[uuid]->exitCode()));
}
...
RHEA_FUNC(sys_shellProcessId) {
    ...
    return DynamicObject(static_cast<double>(shellMap[uuid]->processId()));
}
...
RHEA_FUNC(sys_platform) {
    std::string name = "unknown", variant = "unknown", version = "unknown",
                kernel = "unknown";
...
#include <TargetConditionals.h>
    ...
    return DynamicObject(std::make_shared<std::vector<DynamicObject>>(info));
}

RHEA_FUNC(sys_wordSize) {
    return DynamicObject(static_cast<double>(sizeof(void*) * 8));
}
...
RHEA_FUNC(sys_endianess) {
    ...
    return DynamicObject((*c == 1) ? "little-endian" : "big-endian");
}
...
RHEA_FUNC(sys_cpuFeatures) {
    ...
    return DynamicObject(
        std::make_shared<std::vector<DynamicObject>>(features));
}
...
RHEA_FUNC(sys_sleep) {
    ...
    DynamicObject value = args.at(0);
#if defined(_WIN32) || defined(_WIN64)
    Sleep(static_cast<DWORD>(value.getNumber()));
#else
    std::this_thread::sleep_for(
        std::chrono::milliseconds(static_cast<int64_t>(value.getNumber())));
#endif
